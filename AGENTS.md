...
- QA in the parallel model: developers write their own unit/integration tests and mark their tickets `Ready For QA` when the PR is up. Hermes then dispatches a dedicated QA agent session (fresh context, never the implementing agent) that tests on UAT with per-AC evidence and owns the `In QA` verdict. QA findings go back as `Code Review` with repros; only a QA pass moves the ticket to `Ready For Deploy`.
...
- Security gate (owner directive, Sep 2026 — added after review found limiter/proxy, timing-enumeration, counter-race, and send-before-commit flaws the functional gates missed): any PR touching auth, sessions, password reset, PII, rate limits, or external sends gets a MANDATORY adversarial security pass by a fresh agent before human review. Checklist: authZ enforced server-side on every endpoint (UI gating is never enough); enumeration via response body OR timing; limiter keys resolve the real client IP behind proxies; shared counters increment atomically; external sends happen post-commit; secrets/PII never in logs. Other backend PRs get the same checklist as a light pass. Findings bounce the ticket to `In Progress` like any review blocker.
- Builder self-check (same directive — proves "doesn't break", not just "works"): before opening a PR the implementing agent asserts: the server enforces what the UI gates; counters/flags shared across requests are atomic; external side effects fire after commit; limiter keys use the real client IP; error/retry paths terminate (no dead-end UI states); customer-visible identifiers are returned by the APIs that render them.
- Hardened QA gate (same directive): QA tests edge states and abuse cases, not just happy paths — expired tokens, disabled toggles, parallel/double submits, stale sessions, legacy-null rows. A QA pass that only covers the golden path is incomplete.
...
- Full-stack features build frontend and backend simultaneously when the two sides are independent. When the frontend depends on the backend contract, BACKEND finishes first, FRONTEND builds against the published contract artifact, then each side opens its PR for review. A PR goes up for review only when its side is complete — never mid-build.
- Human cross-review routing (owner directive, Sep 2026): Shree's commits are reviewed by Amol; Amol's commits are reviewed by Shree. Hermes assigns the reviewer on every feature PR and enforces it — review requests never sit unassigned and no one reviews their own side. For agent-implemented work the lean reviewer pass above runs first; the human cross-review is the second gate on the same PR.
