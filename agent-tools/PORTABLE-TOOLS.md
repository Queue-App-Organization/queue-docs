...
Runtime config shapes (generated by `scripts/generate_agent_mcp_configs.py` — never hand-edit the outputs): `.mcp.json` (Claude Code) and `.cursor/mcp.json` (Cursor) use the `mcpServers` object shape those runtimes require; `.vscode/mcp.json` and `.agents/mcp/mcp.local.json` keep the portable `servers` shape. Verified 2026-09-19: `claude mcp list` shows `atlassian`, `github`, `filesystem` pending approval.
